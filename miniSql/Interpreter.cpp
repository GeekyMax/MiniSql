...
int Interpreter::interpreter(const string& sql) {
	int tmp = 0;
	string word;
	...
	if (strcmp(word.c_str(), "create") == 0) {
		...
		if (strcmp(word.c_str(), "table") == 0) {
			...
			while (!word.empty() && strcmp(word.c_str(), "primary") != 0 && strcmp(word.c_str(), ")") != 0) {
				const string attributeName = word;
				int type = 0;
				bool ifUnique = false;
		...
		if (strcmp(word.c_str(), "index") == 0) // create index 
		{
			string indexName;
			string tableName;
