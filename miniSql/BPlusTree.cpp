...
tree::tree(string fileName, int degree, typeenum type, int size):
	...
	switch (type) {
	case INT:
		root = new node<int>(nullptr, degree, 1);
		this->size = sizeof(int);
		break;

	case FLOAT:
		root = new node<float>(nullptr, degree, 1);
		this->size = sizeof(float);
		break;

	case STRING:
		root = new node<string>(nullptr, degree, 1);
		break;
	}
}
...
int tree::read() {
	...
	while (1) {
		...
		switch (type) {
		case INT: insert(tostring(*(int *)a), val);
			break;
		case FLOAT: insert(tostring(*(float *)a), val);
			break;
		case STRING: insert(a, val);
			break;
		}
	}
	return 0;
